...
export default function RawLogsView({
  ...
  const handleToggleCompare = useCallback((entry: LogEntry) => {
    if (compareA && compareA === entry) { setCompareA(null); return; }
    if (compareB && compareB === entry) { setCompareB(null); return; }
    if (!compareA) { setCompareA(entry); return; }
    if (!compareB) { setCompareB(entry); setShowCompare(true); return; }
    ...
    <div className="dashboard">
                    ...
                    {visibleEntries.map((entry, idx) => {
                      ...
                      <div
                        ...
                        style={{
                          ...
                          ...(severity ? { background: severityColors[severity] || undefined, borderRight: `3px solid ${severityBorder[severity] || 'transparent'}` } : {}),
                          ...(entry.sourceFile &&
                          data.sourceFiles &&
                        ...
                        title={severity ? `SIGMA: ${severity} severity` : undefined}
                      >
                          ...
                          <>
                            ...
                            <span className="log-action">
                              ...
                              <button
                                ...
                                title={compareA === entry || compareB === entry ? "Remove from comparison" : "Add to comparison"}
                                style={{
                                  marginLeft: 2,
                                  opacity: (compareA === entry || compareB === entry) ? 1 : 0.5,
                                  color: (compareA === entry || compareB === entry) ? '#a855f7' : undefined,
                                }}
                              >
                                ⚖️
                              </button>
      ...
      {(compareA || compareB) && !showCompare && (
        <div style={{
          ...
          background: "var(--bg-secondary, #1a1a2e)", border: "1px solid rgba(168,85,247,0.3)",
          borderRadius: 10, padding: "8px 16px", display: "flex", alignItems: "center", gap: 12,
          boxShadow: "0 8px 32px rgba(0,0,0,0.5)", zIndex: 1000, fontSize: "0.82rem",
        }}>
          <span style={{ color: "#a855f7" }}>⚖️</span>
          <span style={{ color: "#ccc" }}>
            {compareA ? "1 selected" : "0 selected"}
            {compareB ? " + 1" : ""} — {compareA && compareB ? "Ready" : "Select 2 events with ⚖️ to compare"}
          </span>
          {compareA && compareB && (
            <button onClick={() => setShowCompare(true)} style={{
              background: "rgba(168,85,247,0.15)", border: "1px solid rgba(168,85,247,0.4)",
              borderRadius: 6, padding: "4px 12px", cursor: "pointer", color: "#a855f7", fontSize: "0.8rem",
            }}>Compare</button>
          )}
